  ...
  const observer = new IntersectionObserver((entries) => {
    entries.forEach(entry => {
      if(entry.isIntersecting) {
        ...
        setTimeout(() => entry.target.classList.remove('reveal', 'visible'), 1000);
        observer.unobserve(entry.target);
      }
    });
  }, { threshold: 0.05, rootMargin: "0px 0px -30px 0px" });

  const animatedElements = document.querySelectorAll('.product-card:not(.reveal), .category-card:not(.reveal), .section-header:not(.reveal)');
...
function initTilt() {
  document.querySelectorAll('.product-card:not(.tilt-added), .category-card:not(.tilt-added)').forEach(card => {
    card.classList.add('tilt-added');
    
    card.addEventListener('mousemove', e => {
      // Don't apply tilt on smaller screens for performance
...
// Ensure dynamic components observe the observer when rendered
function observeNewElements() {
  setTimeout(initAnimations, 50);
}

// Override render functions to trigger animations
const _renderFeatured = renderFeatured;
renderFeatured = function() {
  _renderFeatured();
  observeNewElements();
};

const _renderBrowse = renderBrowse;
renderBrowse = function(list = null) {
  _renderBrowse(list);
  observeNewElements();
};
